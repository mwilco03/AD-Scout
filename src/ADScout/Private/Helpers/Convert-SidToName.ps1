function Convert-SidToName {
    ...
    process {
        try {
            ...
            # Try to use externalized SID data if available
            $wellKnownSids = if ($script:ADScoutSidData -and $script:ADScoutSidData.wellKnownSids) {
                # Convert PSCustomObject to hashtable for lookup
                $sidHash = @{}
                $script:ADScoutSidData.wellKnownSids.PSObject.Properties | ForEach-Object {
                    $sidHash[$_.Name] = $_.Value
                }
                $sidHash
            }
            else {
                # Fallback to hardcoded values if data file not loaded
                @{
                    'S-1-0-0'     = 'Nobody'
                    'S-1-1-0'     = 'Everyone'
                    'S-1-2-0'     = 'Local'
            ...
            if ($Sid -match '^S-1-5-21-[\d-]+-(\d+)$') {
                $rid = $Matches[1]
                $domainRids = if ($script:ADScoutSidData -and $script:ADScoutSidData.domainRelativeRids) {
                    # Convert PSCustomObject to hashtable for lookup
                    $ridHash = @{}
                    $script:ADScoutSidData.domainRelativeRids.PSObject.Properties | ForEach-Object {
                        $ridHash[$_.Name] = $_.Value
                    }
                    $ridHash
                }
                else {

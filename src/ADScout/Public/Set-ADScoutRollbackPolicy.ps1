function Get-ADScoutRollbackPolicy {
    ...
    .DESCRIPTION
        Retrieves the configuration for how long rollback data is retained,
        maximum storage limits, and archive settings.

    .EXAMPLE
        Get-ADScoutRollbackPolicy
        Returns the current rollback retention policy settings.

    .OUTPUTS
        PSCustomObject with policy settings.
...
function Set-ADScoutRollbackPolicy {
    ...
    .EXAMPLE
        ...
        Sets 90-day retention with compliance archiving.

    .EXAMPLE
        Set-ADScoutRollbackPolicy -MaxBatches 50 -MaxSizeMB 1000
        Limits to 50 batches or 1GB storage.
...
function Invoke-ADScoutRollbackCleanup {
    ...
    # Identify files to archive/delete based on age
    ...
    if ($toArchive.Count -gt 0 -and $policy.ArchivePath) {
        ...
        if ($PSCmdlet.ShouldProcess($archiveDir, "Archive $($toArchive.Count) files")) {
            ...
            foreach ($file in $toArchive) {
                Copy-Item -Path $file.FullName -Destination (Join-Path $archiveDir $file.Name)
            }
